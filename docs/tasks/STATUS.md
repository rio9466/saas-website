...
| BE-02 | 联系表单与收件箱                                  | `be-contact-inbox`       | `master-relay` | BE-01                   | done | 已合入 2b20723          |
| BE-03 | 用户控制台 API                                    | `be-user-console-api`    | `master-relay` | —                       | todo | worktree 已建，待认领   |
| BE-04 | 管理端内容 UI                                     | `be-admin-content-ui`    | `master-relay` | BE-01、BE-02            | todo | worktree 已建；运行依赖 HOTFIX-01 |
| FE-01 | 前端基础：i18n + 布局 + API 客户端 + SEO          | `fe-foundation`          | `master-relay` | —                       | done | 已合入 f83c373          |
| FE-02 | 公开页面（首页/功能/价格/关于/文档）              | `fe-public-pages`        | `master-relay` | BE-01、BE-02、FE-01     | done | 已合入 fa9a792          |
