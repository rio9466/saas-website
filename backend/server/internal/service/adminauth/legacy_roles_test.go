...
// reservedRolePermissionCodes mirrors the permission sets the pre-20260907
// migrations granted to the reserved roles, so tests exercising multi-role
// behavior keep their original semantics after startup migrations stopped
// seeding those roles on fresh databases.
func reservedRolePermissionCodes(code string) []string {
	switch code {
...
func seedReservedRole(
	...
	codes := reservedRolePermissionCodes(code)
	if len(codes) > 0 {
